...
router.post("/signup", (req, res) => {
  ...
  if (!email || !password || !name) {
    res.status(422).json({ error: "Hamma inputlar to'ldirilgan bo'lishi kerak" });
  }
  User.findOne({ email: email })
    .then((savedUser) => {
      if (savedUser) {
        return res.status(422).json({ error: "Bunday email manzil ro'yhatdan o'tgan" });
      }
...
router.post("/signin", (req, res) => {
  ...
  if(!email || !password) {
    res.status(422).json({error: "Iltimos parol va email manzilingizni kiriting"})
  }
  User.findOne({email: email})
    .then(savedUser => {
      if(!savedUser) {
